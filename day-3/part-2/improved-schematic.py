...
if __name__ == "__main__":
    with open('../engine-schematic.txt') as schematic:
        ...
        # Keep track of where the gears are using their coordinates
        gear_coordinates = {}
            ...
            for char_idx, char in enumerate(schematic_line):
                ...
                if (char == '.' or not char.isdigit() or char_idx == (schematic_line_length - 1)) and possible_part_number:
                    ...
                    if is_special_char_adjacent(adjacent_chars):
                        # We know it's a part number.
                        # Now we need to find out if any of the adjacent 
                        # chars are gears, and if so, stick the part number in 
                        # the gear coordinate dict.
                        part_number = int(possible_part_number)
                        if adjacent_chars['prev']:
                            for offset, c in enumerate(adjacent_chars['prev']):
                                if c == "*":
                                    coordinate = (prev_line_idx, prev_char_idx + offset)
                                    gear_coordinates.setdefault(coordinate, []).append(part_number)
                        if adjacent_chars['left'] == "*":
                            coordinate = (line_idx, prev_char_idx)
                            gear_coordinates.setdefault(coordinate, []).append(part_number)
                        if adjacent_chars['right'] == "*":
                            coordinate = (line_idx, char_idx)
                            gear_coordinates.setdefault(coordinate, []).append(part_number)
                        if adjacent_chars['next']:
                            for offset, c in enumerate(adjacent_chars['next']):
                                if c == "*":
                                    coordinate = (next_line_idx, prev_char_idx + offset)
                                    gear_coordinates.setdefault(coordinate, []).append(part_number)
                    possible_part_number = ''
                    adjacent_chars = {"prev": "", "left": "", "right": "", "next": ""}
        gear_ratio_sum = 0
        ...
        print(f'Done! The gear ratio sum is: {gear_ratio_sum}')
